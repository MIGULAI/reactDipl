...
const AddPosisionForm = ({position, onSubmitForm, submitButtonValue = 'Додати'}) => {
    ...
    <div className={myClasses.formOneRow}>
        <div className={classes.columItem}>
            <div className={classes.inputFuild}>
                <MyLabel >Назва посади :</MyLabel>
                <MyFormInput
                    type="text"
                    placeholder={'Назва видавця'}
                    register={{ ...register('positionName', { required: true }) }}
                />
                {
                    errors.positionName && <span>Введіть назву посади</span>
                }
            </div>
        </div>
...
AddPosisionForm.propTypes = {
    onSubmitForm: PropTypes.func,
    submitButtonValue: PropTypes.string
}


export default AddPosisionForm